...
function App() {
  ...
  const [currentModel, setCurrentModel] = useState(
    Cookies.get("GPT_CURRENT_MODEL") || "gpt-3.5-turbo"
  );
  ...
  const renderChatboxRoot = (msgs, chat) => {
    ...
    root.render(
      msgs[chat]?.map((message, idx) => (
        <>
          {typeof message.reasoning != "undefined" && message.reasoning !== "" && (
            <div className="reasoning" id={message.role + ".reasoning"}>
              {"🤔：" + message.reasoning || ""}
            </div>
          )}
          <div
  ...
  const sendMessageToModel = async (updatedMessages) => {
    ...
    if (currentModel === "dall-e-2" || currentModel === "dall-e-3") {
      const assistantData = await sendRequest(
        preURL,
        currentModel,
        apikey,
        chatMsgs
      ...
      await sendRequest(
        ...
        (reasoningDelta, contentDelta) => {
          ...
          if (typeof contentDelta == "undefined") contentDelta = "";
          if (typeof reasoningDelta == "undefined") reasoningDelta = "";
          msgs[currentChat][i].content += contentDelta || "";
          msgs[currentChat][i].reasoning += reasoningDelta || "";
          setMessages(msgs);
